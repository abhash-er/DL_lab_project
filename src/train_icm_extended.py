...
from utils import check_dir, CustomDataset, CaptionDataset, ExtendedCustomDataset
...
if __name__ == "__main__":
    ...
    # Load Training Data
    small_train_data = CustomDataset(data_file, train=True)
    
    # Load Extended training data
    extended_train_data = ExtendedCustomDataset()
    # sample few of them 
    sampled_list = random.sample(range(0,len(extended_train_data)), 300 * 16)
    extended_subset_train_data = torch.utils.data.Subset(extended_train_data, sampled_list)
    train_data = torch.utils.data.ConcatDataset([small_train_data, extended_subset_train_data])
